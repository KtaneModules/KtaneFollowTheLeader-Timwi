...
public class FollowTheLeaderModule : MonoBehaviour
{
    ...
    private static readonly Delegate[] _tableRules = Ex.NewArray<Delegate>(
        ...
        new MakeSimpleRule(name => new RuleInfo { Name = name, Phrasing = "Two of the previous three wires share a color.", Evaluate = (w, ix, num, prev) => w(ix - 3).WireColor == w(ix - 2).WireColor || w(ix - 3).WireColor == w(ix - 1).WireColor || w(ix - 2).WireColor == w(ix - 1).WireColor }),
        new MakeSimpleRule(name => new RuleInfo { Name = name, Phrasing = "The previous three wires have different colors.", Evaluate = (w, ix, num, prev) => !(w(ix - 3).WireColor == w(ix - 2).WireColor || w(ix - 3).WireColor == w(ix - 1).WireColor || w(ix - 2).WireColor == w(ix - 1).WireColor) }),
        new MakeSimpleRule(name => new RuleInfo { Name = name, Phrasing = "The previous three wires are all the same color.", Evaluate = (w, ix, num, prev) => w(ix - 3).WireColor == w(ix - 2).WireColor && w(ix - 3).WireColor == w(ix - 1).WireColor }),
